...
def get_tasks():
    ...
    if cached:
        print(" Servi depuis le cache Redis")
        return jsonify(json.loads(cached))

    # Sinon aller chercher dans PostgreSQL
    tasks = Task.query.all()
    result = [t.to_dict() for t in tasks]

    # Sauvegarder dans Redis pour 30 secondes
    cache.setex("tasks", 30, json.dumps(result))
    print(" Servi depuis PostgreSQL")
...
def add_task():
    ...
    db.session.add(task)
    db.session.commit()
    cache.delete("tasks")  # Invalider le cache
